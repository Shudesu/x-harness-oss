...
Production GAS/Resolve examples, account row id, canonical LP mapping, environment topology, read-only metrics source, and canonical song/member masters remain explicit non-blocking entries in `docs/Decision-Grill.md`. Deterministic generation, evidence-derived seven-axis scoring, atomic audit persistence, two-factor human proof, weighted X text and build-time configuration projection are resolved and enforced in code/CI.

The automated suite covers domain, contract, adapter and fail-closed E2E behavior plus actual Hono routes and D1 semantics under Miniflare. `pnpm validate:production-inputs` validates real structured inputs without printing contents, and `pnpm smoke:staging` verifies deployed draft-only capabilities and legacy-write locks. Production preflight requires explicit success attestations for both; current automated tests intentionally make no cloud or X calls.

The smoke runner was also exercised against the real Wrangler Worker bundle and local D1 schema: health and capabilities returned 200, a legacy post mutation returned 423, and an inbox read without human proof returned 403. This validates the runner itself but does not substitute for a deployed staging run.
